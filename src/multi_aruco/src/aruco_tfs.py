...
class aruco_pos_sim():
    ...
    def publish_tfs(self, timer_event):
        ...
        self.broadcaster.sendTransform(TransformStamped(msg.Header(0, rospy.Time.now(), "base_link"), "camera_link", Transform(Vector3(0, 0, 0), Quaternion(0, 0, 0, 1))))
        self.broadcaster.sendTransform(TransformStamped(msg.Header(0, rospy.Time.now(), "camera_link"), "camera_color_optical_frame", Transform(Vector3(0, 0, 0), Quaternion(0.5, 0.5, 0.5, -0.5))))
